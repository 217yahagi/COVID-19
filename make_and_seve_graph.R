...
data_all <- read_csv("prefectures_all_1022.csv", locale = locale(encoding = "SHIFT-JIS"))
head(data_all)
...
make_series <- function(i,maxdate){
  ...
  ##plot(p)
  ggsave(file = paste("image/",file_name[i],".png",sep=""), plot = p)
  return(p)
#  ggsave(file = paste("image/",file_name[i],".png"), plot = p)
